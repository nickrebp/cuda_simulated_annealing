...
rb_red_blk_node* RBLTEQuery(rb_red_blk_tree* tree, void* q) {
  rb_red_blk_node* x=tree->root->left;
  rb_red_blk_node* nil=tree->nil;
  rb_red_blk_node *greatest = nil;
  int compVal;
  if (x == nil) return(0);
  compVal=tree->Compare(x->key,(int*) q);
  while(0 != compVal) {/*assignemnt*/
    if (1 == compVal) { /* x->key > q */
      ...
      //if requesting the right child, this node is the greatest that is less
      //than the query so far
      greatest = x;
      x=x->right;
    }
    if (x == nil) {
...
rb_red_blk_node* RBGTEQuery(rb_red_blk_tree* tree, void* q) {
  rb_red_blk_node* x=tree->root->left;
  rb_red_blk_node* nil=tree->nil;
  rb_red_blk_node *greatest = nil;
  int compVal;
  ...
  while(0 != compVal) {/*assignemnt*/
    if (1 == compVal) { /* x->key > q */
      ...
      //if requesting the right child, this node is the greatest that is less
      //than the query so far --> see above for the greatest assignment
      //greatest = x;
      x=x->right;
    }
    if (x == nil) {
...
//returns 0 if the trees contain identical elements 
//(equivalent key as par the supplied compare function)
char RBTreeCompare(rb_red_blk_tree *tree1, rb_red_blk_tree *tree2, 
		   int (NodeEquals)(void *, void *)) {
  rb_red_blk_node *node1 = TreeFindLeast(tree1), 
    *node2 = TreeFindLeast(tree2);
  if (node1 == NULL || node2 == NULL)
    return node1 != node2;

  while (node1 != tree1->nil || node2 != tree2->nil) {
    if (node1 == tree1->nil || node2 == tree2->nil || !NodeEquals(node1->key, node2->key))
      return 1;
    node1 = TreeSuccessor(tree1, node1);
    node2 = TreeSuccessor(tree2, node2);
  }
  return 0;
}
...
rb_red_blk_tree *RBTreeCopy(rb_red_blk_tree *tree, void *(*KeyCopy)(void *),
			    void *(*InfoCopy)(void *)) {
  rb_red_blk_tree *tree2;
  tree2 = RBTreeCreate(tree->Compare, tree->DestroyKey, 
					tree->DestroyInfo, tree->PrintKey,
					tree->PrintInfo);
  rb_red_blk_node *node = TreeFindLeast(tree);
  while (node != NULL && node != tree->nil) {
    if (node->key) //do not copy nodes with NULL key
      RBTreeInsert(tree2, KeyCopy(node->key), InfoCopy(node->info));
...
unsigned int RBIsTreeEmpty(rb_red_blk_tree *tree) {
  return tree->root->left == tree->nil && tree->root->right == tree->nil;
}
